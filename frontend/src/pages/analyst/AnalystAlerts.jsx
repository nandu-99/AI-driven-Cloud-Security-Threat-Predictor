...
import { getAnalystAlerts, escalateAlert, markInvestigated } from '../../services/api.js'
...
export default function AnalystAlerts() {
  ...
  useEffect(() => {
    getAnalystAlerts().then(setAlerts)
    const id = setInterval(() => getAnalystAlerts().then(setAlerts), 5000)
    ...
    <div style={{ padding: 20 }}>
      ...
      {alerts.map((a, i) => (
        <div key={i} style={{
          ...
          padding: 14,
          marginBottom: 12,
          borderLeft: `3px solid ${a.level === 'critical' || a.level === 'high' ? 'var(--red)' : a.level === 'medium' ? 'var(--yellow)' : 'var(--neon)'}`,
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, alignItems: 'center' }}>
            <span style={{ color: 'var(--cyan)', fontWeight: 600, fontSize: 13 }}>{a.id}</span>
            <Badge variant={a.level === 'critical' || a.level === 'high' ? 'red' : a.level === 'medium' ? 'yellow' : 'green'}>
              {a.level.toUpperCase()}
            </Badge>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
            <Button
              variant="cyan"
              onClick={() => {
                if (investigated[a.id]) return
                markInvestigated({ userId: a.id, note: 'Investigating active alert' }).then(() => {
                  setInvestigated(prev => ({ ...prev, [a.id]: true }))
                  notify(`Investigated ${a.id}`)
                })
              }}
            >
              {investigated[a.id] ? 'Investigated' : 'Investigate'}
            </Button>
            <Button
              variant="yellow"
              onClick={() => {
                if (escalated[a.id]) return
                escalateAlert({ userId: a.id, reason: 'Alert escalation' }).then(() => {
                  setEscalated(prev => ({ ...prev, [a.id]: true }))
                  notify(`Escalated ${a.id}`)
                })
              }}
            >
