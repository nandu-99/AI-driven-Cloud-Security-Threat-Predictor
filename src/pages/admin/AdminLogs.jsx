...
import { FAILED_TREND, FILE_ACCESS_DATA, MOCK_USERS } from '../../mock-data/mockData.js'
...
const RISK_DIST = [
  { name: 'High',   value: MOCK_USERS.filter(u => u.risk === 'High').length },
  { name: 'Medium', value: MOCK_USERS.filter(u => u.risk === 'Medium').length },
  { name: 'Low',    value: MOCK_USERS.filter(u => u.risk === 'Low').length },
]
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div style={{ background: '#0d1426', border: '1px solid #1e3a4a', padding: '6px 12px', borderRadius: 5, fontSize: 11, color: '#e2e8f0' }}>
        <span style={{ color: RISK_PIE_COLORS[['High','Medium','Low'].indexOf(d.name)] }}>{d.name}</span>: {d.value} users
      </div>
    )
  }
  return null
